...
public class WheelView extends View {
    ...
    // 条目间距倍数
    static final float lineSpacingMultiplier = 3.4F;
    boolean isLoop;
    ...
    public WheelView(Context context, AttributeSet attrs) {
        ...
        dividerColor = getResources().getColor(R.color.logo_color);
        //配合customTextSize使用，customTextSize为true才会发挥效果
        textSize = getResources().getDimensionPixelSize(R.dimen.pickerview_textsize);
        textsizelable=getResources().getDimensionPixelSize(R.dimen.pickerview_textsize_lable);
    ...
    private void initPaints() {
        ...
        paintCenterTextlable = new Paint();
        paintCenterTextlable.setColor(textColorCenter);
        paintCenterTextlable.setAntiAlias(true);
        paintCenterTextlable.setTextScaleX(1.1F);
        paintCenterTextlable.setTypeface(Typeface.MONOSPACE);
        paintCenterTextlable.setTextSize(textsizelable);
    ...
    protected void onDraw(Canvas canvas) {
        ...
        //单位的Label
        if(label != null) {
            int drawRightContentStart = measuredWidth - getTextWidth(paintCenterTextlable,label);
            //靠右并留出空隙
            canvas.drawText(label, drawRightContentStart - CENTERCONTENTOFFSET-20, centerY, paintCenterTextlable);
        }
        counter = 0;
        while (counter < itemsVisible) {
            canvas.save();
            ...
            if (angle >= 90F || angle <= -90F) {
                ...
                if (translateY <= firstLineY && maxTextHeight + translateY >= firstLineY) {
                    ...
                    canvas.drawText(contentText, drawOutContentStart-30, maxTextHeight, paintOuterText);
                    canvas.restore();
                    canvas.save();
                    canvas.clipRect(0, firstLineY - translateY, measuredWidth, (int) (itemHeight));
                    canvas.scale(1.0F, (float) Math.sin(radian) * 1F);
                    canvas.drawText(contentText, drawCenterContentStart-30, maxTextHeight - CENTERCONTENTOFFSET, paintCenterText);
                    canvas.restore();
