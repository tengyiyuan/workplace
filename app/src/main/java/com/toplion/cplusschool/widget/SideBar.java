...
public class SideBar extends View {
    ...
    protected void onDraw(Canvas canvas) {
        ...
        for (int i = 0; i < A_Z.length; i++) {
            paint.setColor(getResources().getColor(R.color.logo_color));  //设置字体颜色
            paint.setTypeface(Typeface.DEFAULT_BOLD);  //设置字体
            paint.setAntiAlias(true);  //设置抗锯齿
            paint.setTextSize(35);  //设置字母字体大小
            // 选中的状态
            if (i == choose) {
                paint.setColor(getResources().getColor(R.color.red));  //选中的字母改变颜色
...
//                setBackgroundDrawable(new ColorDrawable(getResources().getColor(R.color.actionBarText)));
                choose = -1;//
                invalidate();
                if (mTextDialog != null) {
